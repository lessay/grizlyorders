...
class Product:
    ...
    def from_url(cls, url: str, session_cookie: str) -> 'Product':
        response = requests.get(url, cookies={'4every1_ses': session_cookie})
        response.raise_for_status()

        document = lxml.html.fromstring(response.text)

        if len(id_elem := document.cssselect('#product_detail_content form input')) == 0:
            raise OutOfStock('Can not get id')

        weight_match = re.match('(\d+)\s+g', document.cssselect('#product_detail_content a.js-package_item.active > span')[0].text)
        weight = int(weight_match[1]) if weight_match else 0

        return Product(
            id=id_elem[0].attrib['data-pid'],
            name=document.cssselect('#product_detail_content h1')[0].text,
            price=Decimal(re.match(r'\s*(\d+(?:,\d+)?)\s*Kč', document.cssselect('#product_detail_content div.primary')[0].text)[1]),
            weight=weight,
            url=url,
        )
...
def order_from_sheet(google_auth:str, sheet_id: str, worksheet_name: str, session_cookie: str) -> dict[Product, int]:
    ...
    links_and_counts = wsheet.get(f'F3:G{MAX_ROWS}')

    # strikethrough all prices
    wsheet.format(f"D3:D{MAX_ROWS}", {"textFormat": {"strikethrough": True}})

    ordered_products: dict[Product, int] = {}
    for row, (url, quantity) in enumerate(links_and_counts, start=3):
